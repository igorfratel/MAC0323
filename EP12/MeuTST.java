...
/*======================================================================================================================== */    
    ...
    public Iterable<String> keysWithPrefixByValue(String prefix) {
        ...
        Queue<String> queue = new Queue<String>();
        MaxPQ<KeyVal> myPQ = new MaxPQ<KeyVal>();
        Node<Value> x = get(root, prefix, 0);
        if (x == null) return queue;
        if (x.val != null) queue.enqueue(prefix);
        collect(x.mid, new StringBuilder(prefix), queue);
        for (String T: queue) {
            KeyVal myKeyVal = new KeyVal(T, get(T));
            myPQ.insert(myKeyVal); 
        }
        Queue<String> newQueue = new Queue<String>();
        for (KeyVal K: myPQ) {
            newQueue.enqueue(K.getKey());
        }
        return newQueue;
    ...
    private class KeyVal<Value extends Comparable<Value>> implements Comparable<KeyVal<Value>> {
        private String key;
        private Value val;
        public KeyVal(String keye, Value vale) {
            key = keye;
            val = vale;
        }
        ...
        public Value getVal() {
         ...
         public int compareTo(KeyVal<Value> that) {
...
/*=========================================================================================================================
    ...
    private Node delete(Node x, String key, int d) {
        if (x == null) return null;
            ...
            if (x.mid == null && x.left == null && x.right == null) x = null; /*may be dangerous!*/
        }
        return x;//????
    }
